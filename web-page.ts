...
export const WEB_SECTION_IDS = ["needs", "sessions", "activity", "github", "recent", "usage", "localAi", "machine", "changes", "projects", "containers"] as const;
export const DEFAULT_NARROW_ORDER = ["sessions", "changes", "needs", "projects", "activity", "github", "recent", "usage", "localAi", "machine", "containers"];
...
export function parseThemeColors(raw: string): ThemeColors {
  ...
  const pick = (key: string, fallback: string) => {
    const match = raw.match(new RegExp(`(?:^|\\n)${key}\\s*=\\s*"?(#(?:[0-9a-fA-F]{6}|[0-9a-fA-F]{3}))"?`));
    return match && /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(match[1]) ? match[1] : fallback;
  };
  theme.background = pick("background", theme.background);
  theme.foreground = pick("foreground", theme.foreground);
  theme.green = pick("color2", theme.green);
  theme.yellow = pick("color3", theme.yellow);
  theme.red = pick("color1", theme.red);
  theme.blue = pick("color4", theme.blue);
  theme.magenta = pick("color5", theme.magenta);
  theme.cyan = pick("color6", theme.cyan);
  return theme;
}
...
function renderMeterBar(label: string, value: string, fraction: number, fill: string, rawLabel = false): string {
...
function heatColor(kind: string, activity: boolean): string {
  if (!activity) {
    if (kind === "commit") return GREEN;
    if (kind === "pr") return "#c678dd";
    if (kind === "review") return "#56b6c2";
    if (kind === "issue") return YELLOW;
    if (kind === "comment") return BLUE;
    return "#abb2bf";
  }
  return providerColorHex(kind);
}

function renderHeat(cells: unknown, days: unknown, kinds: string[], activity: boolean): string {
  ...
  for (let d = 0; d < 7; d++) {
    ...
    for (let h = 0; h < 24; h++) {
      ...
      const color = n > 0 ? heatColor(kind || kinds[0] || "", activity) : "var(--fg)";
...
export function renderUsageSection(snap: any, prefs: DashPrefs = parseDashPrefs({})): string {
  ...
  const charts: string[] = [];
  if (tokenSeries.length) charts.push(`<div class="chart"><div class="chart-label">TOKENS · 7 days</div>${renderTrendSvg(tokenSeries, days, n => fmtTokens(n))}</div>`);
  ...
  for (const key of keys) {
    ...
    blocks.push(`<div class="subcard" style="border-color:${tone}44"><div class="usage-head"><span class="tag" style="color:${tone}">${escapeHtml(row.name || key, 40)}</span> <span class="meta">${escapeHtml(row.tierLabel, 32)}</span></div><div class="meta">today ${escapeHtml(row.todayPrompts || 0, 12)}p · ${escapeHtml(fmtTokens(row.todayTotalTokens), 16)} tok${escapeHtml(todayValue, 24)}</div>${status ? `<div class="meta">${escapeHtml(status, 200)}</div>` : ""}${life.length ? `<div class="meta">${escapeHtml(life.join(" · "), 220)}</div>` : ""}${limits}</div>`);
  }
  return card("USAGE &amp; LIMITS", blocks.join(""), "", "usage", m.on, m.order);
}

export function renderMachineSection(snap: any, prefs: DashPrefs = parseDashPrefs({})): string {
  ...
  parts.push(renderMeterBar("CPU", cpuBits.join(" · "), (Number.isFinite(cpuPct) ? cpuPct : 0) / 100, cpuPct > 85 ? RED : BLUE));
  parts.push(renderMeterBar("RAM", ramBits.join(" · "), (Number.isFinite(ramPct) ? ramPct : 0) / 100, ramPct > 90 ? RED : GREEN));
  for (const disk of disks) {
    ...
    parts.push(renderMeterBar(labelHtml, value, (Number.isFinite(pct) ? pct : 0) / 100, pct > 90 ? RED : YELLOW, true));
  }
  ...
  parts.push(renderMeterBar(wifiLabelHtml, wifiVal, wifiFrac, wifiFill, true));
...
function renderSessions(snap: any, prefs: DashPrefs = parseDashPrefs({})): string {
  ...
  const cards = sessions.map((item: any) => {
    const tone = providerColorHex(item.provider);
...
function renderNeeds(snap: any, prefs: DashPrefs = parseDashPrefs({})): string {
  ...
  const rows = attention.map((item: any) => {
    ...
    const tone = kind === "blocked" ? RED : kind === "waiting" ? YELLOW : GREEN;
...
function projectStatus(item: any): { label: string; tone: string } {
  const status = String(item.status || "");
  if (status === "blocked") return { label: "BLOCKED", tone: RED };
  if (status === "running") return { label: "RUNNING", tone: BLUE };
  if (status === "behind" || status === "changed") return { label: status.toUpperCase(), tone: YELLOW };
  if (status === "unknown") return { label: "UNKNOWN", tone: "#888" };
  return { label: "CLEAN", tone: GREEN };
}

function renderProjects(snap: any, prefs: DashPrefs = parseDashPrefs({})): string {
...
function renderActivity(snap: any, prefs: DashPrefs = parseDashPrefs({})): string {
  ...
  const kinds = ["claude", "codex", "grok", "opencode", "pi", "gemini", "ollama"];
  const m = meta("activity", prefs);
  return card("ACTIVITY · LAST 7 DAYS", renderHeat(heat.cells, heat.days, kinds, true), hint, "activity", m.on, m.order);
}

function renderGithub(snap: any, prefs: DashPrefs = parseDashPrefs({})): string {
  ...
  const hint = parts.length ? "today/week · " + parts.join(" · ") : "";
  const body = renderHeat(github.cells, github.days, kinds, false);
  const m = meta("github", prefs);
  return card("GITHUB · LAST 7 DAYS", body, hint, "github", m.on, m.order);
}

function renderRecent(snap: any, prefs: DashPrefs = parseDashPrefs({})): string {
...
function renderLocalAi(snap: any, prefs: DashPrefs = parseDashPrefs({})): string {
  ...
  const chips = loaded.length
    ? loaded.map(name => `<span class="chip" style="color:${GREEN};border-color:${GREEN}">${escapeHtml(name, 48)}</span>`).join("")
...
function renderContainers(snap: any, prefs: DashPrefs = parseDashPrefs({})): string {
  const pack = snap.containers && typeof snap.containers === "object" ? snap.containers : {};
  const m = meta("containers", prefs);
  if (!pack.present) return card("CONTAINERS", `<p class="meta">no container engine</p>`, "", "containers", m.on, m.order);
  const items = take(pack.items, 8);
  if (!items.length) return card("CONTAINERS", `<p class="meta">none</p>`, String(pack.engine || "docker"), "containers", m.on, m.order);
  const rows = items.map((item: any) => {
    const on = !!item.running;
    const label = item.label || item.service || item.name || item.id;
    const tone = on ? GREEN : "#888";
    return `<div class="row"><span class="tag" style="color:${tone}">${on ? "ON" : "OFF"}</span> <span>${escapeHtml(label, 48)}</span><span class="meta">${escapeHtml(item.health || item.state || "", 24)}</span></div>`;
  }).join("");
...
function pageCss(theme: ThemeColors, hasBackground: boolean): string {
  const bg = hexColor(theme.background, FALLBACK_THEME.background);
  const fg = hexColor(theme.foreground, FALLBACK_THEME.foreground);
  const wall = hasBackground ? `url("bg")` : "none";
  return `
:root { color-scheme: dark; --bg:${bg}; --fg:${fg}; --card:${hexToRgba(bg, 0.62)}; --border:${hexToRgba(fg, 0.14)}; --dim:${hexToRgba(fg, 0.62)}; --faint:${hexToRgba(fg, 0.38)}; --gap:8px; --pad:16px; --radius:6px; --scale:1; }
...
.wall { position:fixed; inset:0; background-color:var(--bg); background-image:${wall}; background-position:center; background-size:cover; background-repeat:no-repeat; opacity:0.32; pointer-events:none; }
.stage { position:relative; min-height:100vh; box-sizing:border-box; padding:12px 16px 16px; zoom:var(--scale); }
.desk { display:flex; flex-direction:column; gap:var(--gap); min-height:calc(100vh - 28px); max-width:1920px; margin:0 auto; }
...
.icon-btn { color:${BLUE}; border-color:var(--border); background:transparent; cursor:pointer; font-family:inherit; font-weight:700; }
...
export function renderPage(snap: any, refreshPath: string, nonce = "", prefs: DashPrefs = parseDashPrefs({}), theme: ThemeColors = FALLBACK_THEME, hasBackground = false): string {
  ...
  const opsHtml = prefs.opsOrder.map(id => {
    ...
    if (id === "needs") return renderNeeds(snap, prefs);
    if (id === "projects") return renderProjects(snap, prefs);
    return "";
  }).join("");
  const rightHtml = prefs.rightOrder.map(id => {
    if (id === "usage") return renderUsageSection(snap, prefs);
    if (id === "localAi") return renderLocalAi(snap, prefs);
    if (id === "machine") return renderMachineSection(snap, prefs);
    if (id === "containers") return renderContainers(snap, prefs);
    return "";
  }).join("");
  ...
  rows.push(renderSessions(snap, prefs));
  rows.push(`<div class="ops">${opsHtml}</div>`);
  rows.push(`<div class="heats">${renderActivity(snap, prefs)}${renderGithub(snap, prefs)}</div>`);
  rows.push(renderRecent(snap, prefs));
