...
function renderContainers(snap: any, prefs: DashPrefs = parseDashPrefs({}), theme: ThemeColors = FALLBACK_THEME): string {
  ...
  const rows = items.map((item: any) => {
    ...
    const tone = on ? themeRole(theme, "green") : hexToRgba(themeRole(theme, "foreground"), 0.38);
...
export function renderPage(snap: any, refreshPath: string, nonce = "", prefs: DashPrefs = parseDashPrefs({}), theme: ThemeColors = FALLBACK_THEME, hasBackground = false): string {
  ...
  const rightHtml = prefs.rightOrder.map(id => {
    ...
    if (id === "containers") return renderContainers(snap, prefs, theme);
    return "";
  }).join("");
