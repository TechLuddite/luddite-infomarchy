...
describe("history collection", () => {
  ...
  test("reads Hermes sessions from a fake state.db and skips cron", async () => {
    ...
    mkdirSync(home, { recursive: true });
    const proc = Bun.spawn([process.execPath, join(import.meta.dir, "collector.ts")], {
      env: { HOME: home, USER: "tester", HERMES_HOME: hermesHome, XDG_STATE_HOME: join(home, "state"), PATH: process.env.PATH || "", INFOMARCHY_SKIP_EXTERNAL_IP: "1", INFOMARCHY_SKIP_GITHUB: "1", INFOMARCHY_SKIP_CONTAINERS: "1" },
      stdout: "pipe",
      stderr: "pipe",
    });
    const output = await new Response(proc.stdout).text();
...
describe("second-reviewer findings (2026-09-04)", () => {
  ...
  test("usage caches are normalized to displayed fields with hard bounds", () => {
    ...
    expect(claudeOauthExpiredAt("", 1)).toBe(false);
  });
...
describe("Cursor", () => {
  ...
  test("chats, prompts and projects reach the snapshot, honouring CURSOR_HOME", async () => {
    ...
    mkdirSync(home, { recursive: true });
    const proc = Bun.spawn([process.execPath, join(import.meta.dir, "collector.ts")], {
      env: { HOME: home, USER: "tester", CURSOR_HOME: root, XDG_STATE_HOME: join(home, "state"), PATH: process.env.PATH || "", INFOMARCHY_SKIP_EXTERNAL_IP: "1", INFOMARCHY_SKIP_GITHUB: "1", INFOMARCHY_SKIP_CONTAINERS: "1" },
      stdout: "pipe",
      stderr: "pipe",
    });
    const output = await new Response(proc.stdout).text();
