...
Item {
  ...
  Item {
    ...
    ColumnLayout {
            ...
            Repeater {
              ...
              delegate: Rectangle {
                ...
                ColumnLayout {
                  ...
                  PlainText { Layout.fillWidth: true; Layout.minimumWidth: 0; visible: !!sc.modelData.cwd; text: view.displayPath(sc.modelData.cwd || ""); color: view.textFaint; font.family: view.mono; font.pixelSize: Style.font.caption; elide: Text.ElideMiddle }
                  PlainText { Layout.fillWidth: true; Layout.minimumWidth: 0; visible: (sc.modelData.hosts || []).length > 0; text: "hosted in " + view.sessionHostLabel(sc.modelData) + (sc.modelData.window ? " · click jumps to the pane" : ((sc.modelData.hosts || []).some(function(h) { return h && h.kind === "background" && h.attachId }) ? " · click attaches a terminal" : " · no client window found")); color: sc.tone; font.family: view.mono; font.pixelSize: Style.font.caption; elide: Text.ElideRight }
                  PlainText { Layout.fillWidth: true; Layout.minimumWidth: 0; visible: !!sc.modelData.topic && !!(sc.modelData.window && sc.modelData.window.title); text: sc.modelData.window ? (sc.modelData.window.title || "") : ""; color: view.textDim; font.family: view.mono; font.pixelSize: Style.font.caption; elide: Text.ElideRight }
                  PlainText { Layout.fillWidth: true; Layout.minimumWidth: 0; visible: !!sc.modelData.git; text: sc.modelData.git ? ("git " + sc.modelData.git.branch + (sc.modelData.git.dirty ? " · " + sc.modelData.git.dirty + " changed" : " · clean") + (sc.modelData.git.ahead ? " · ↑" + sc.modelData.git.ahead : "") + (sc.modelData.git.behind ? " · ↓" + sc.modelData.git.behind : "") + (sc.modelData.git.conflicts ? " · " + sc.modelData.git.conflicts + " conflicts" : "")) : ""; color: sc.modelData.git && sc.modelData.git.conflicts ? view.desk.red : sc.modelData.git && sc.modelData.git.dirty ? view.desk.yellow : view.desk.green; font.family: view.mono; font.pixelSize: Style.font.caption; elide: Text.ElideRight }
      ...
      // RIGHT COLUMN: usage + local AI + machine + containers + media
      GridLayout {
        id: rightColumn
        visible: view.sectionEnabled("usage") || view.sectionEnabled("localAi") || view.sectionEnabled("machine") || view.sectionEnabled("containers") || view.sectionEnabled("media")
        Layout.fillHeight: true
  ...
  Rectangle {
    ...
    ColumnLayout {
      ...
      AboutLink { label: "REPOSITORY"; url: view.desk.repoUrl; caption: "github.com/TechLuddite/luddite-infomarchy" }
