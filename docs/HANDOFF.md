...
The focused proposal is local branch `upstream/web-mode` at `321e62f`, in the separate `~/Projects/luddite-infomarchy-web-upstream` worktree, based on upstream `f8447af`. It preserves upstream identity and excludes unrelated fork modules. That exact branch passed 247 tests across 20 files plus isolated browser/QML checks; the fork fix suite passed 286 tests across 21 files. Do not mix these test counts or deploy the upstream extraction over this fork. Review material is local under `.git/web-mode-pr-review/` in the development checkout. An upstream submission still requires explicit agreement.
