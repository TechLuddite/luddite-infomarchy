...
Publication record: [fork PR #22](https://github.com/TechLuddite/luddite-infomarchy/pull/22) targets the fork's `master`, with creation and merge explicitly authorized by the user. Consult the PR for the final merge commit. Upstream submission was explicitly authorized after excluding the experimental bar widget; see the current publication record below. Its review must include README setup changes and both HANDOFF and WEB-MODE-SECURITY; do not publish local PKI, runtime state or credential-bearing test artifacts.
...
## Upstream preparation fixes — 2026-09-10

Unknown explicit saved access modes now keep WEB off until the user selects a supported mode; a missing legacy mode retains LAN compatibility. Settings helper launch failure is detected with a bounded startup check (Quickshell exposes no launch-error signal), clears pending writes, reloads saved state and permits retry. LAN address selection follows default-route metrics and excludes non-default virtual bridges. QR/clipboard failure is reported with dependency guidance; COPY URL uses credential-free status from `copy-url`. New actual QML regressions cover mode rejection, legacy loading and failed launch followed by successful retry; route/tool tests cover bridges and missing executables. `bun test`: 286 passed across 21 files. These preparation fixes were subsequently deployed to the separate live fork with backups and the shell reloaded at the user’s request. Tailscale reported ready and an authenticated HTTPS HEAD returned 200 with normal certificate verification. The user authorized a fork PR and merge for these fixes. The separate upstream proposal remains local and unsubmitted.


## Separate upstream proposal — approved scope

The user explicitly authorized final preparation, fork PR/merge and upstream submission on 2026-09-10, provided the agreed scope does not materially change. The proposal is `upstream/web-mode` in `~/Projects/luddite-infomarchy-web-upstream`, based on upstream `f8447af`. It preserves upstream identity and excludes unrelated fork modules and the experimental settings bar widget. README setup, the private-CA/IP certificate walkthrough, this proposal’s maintenance handoff and WEB-MODE-SECURITY are included. The upstream branch is a separate extraction: never merge it into fork master or deploy it over the live fork.

After removing the widget entrypoint, manifest registration and setup references, the extracted branch passed `bun test --timeout 30000`: 247 tests across 20 files. The fork suite was rerun successfully: 286 tests across 21 files; keep those distinct. Review artifacts are local under `.git/web-mode-pr-review/`. Final extracted commit: `95f1e24`; diff: 27 files (+4,485/−99). Repeated isolated Chromium 390px/privacy on-off-on refresh and actual offscreen QML settings checks passed. README baseline was mechanically checked outside the additive Web sections and optional privacy binding. Native clipboard success and fresh phone installation of the extraction remain unverified. Publication links follow below.
...
## Upstream 1.4.1 integration — 2026-09-11

- Integrated upstream `9726812` on `merge/upstream-1.4.1`, preserving upstream ancestry and fork identity. Previously contributed Pi, media, Ollama-origin, animated-wallpaper and hardening features retain the fork implementations. New behavior is Muse and Antigravity / `agy` live-session detection with labels/colors and Antigravity daemon exclusions; no history or usage formats are invented.
- Added upstream QML syntax and real-import resolution gates. Fork ceilings cover all nine QML files, including SettingsBody and SettingsPanel. Existing findings include dynamic Style properties, unqualified accesses and Quickshell type metadata limitations; these are regression ceilings, not zero-warning lint or visual validation.
- `bun test --timeout 30000`: 308 passed, 0 failed across 23 files, including real offscreen QML settings/lifecycle regressions and TLS tests. Both new QML gates ran with installed imports. In a temporary copy, an injected undefined reference passed syntax but failed resolution (InfoSettings findings 3 versus ceiling 2). `git diff --check` passed.
- The first suite run exposed a pre-existing Grok fixture crossing midnight, compounded by another test changing the parent timezone. Pinned that fixture and its child collector to UTC with both samples on the same day; production usage logic is unchanged.
- Integration is local only; fork master, the live plugin, and the separate upstream Web Mode proposal have not been changed. No live provider or visual desktop verification was performed.

### Live deployment and fork synchronization — 2026-09-11

The user authorized live deployment and shell reload, then synchronization of the live clone, local checkout and fork. Deployed all eight integration files with backups under `$XDG_STATE_HOME/infomarchy/deployment-backups/20260911-003222-upstream-1.4.1`, preserving unrelated live content. `omarchy restart shell` succeeded; shell ping and dashboard geometry IPC responded, and noncredential Web status reported Tailscale running/ready. All repository-tracked live files matched the tested integration before synchronization. The live-only root `HANDOFF.md` note is preserved outside the tracked source set. The separate upstream Web Mode proposal remains unchanged.

## Upstream e2028e5 integration — 2026-09-18

- Merged upstream `e2028e5` (29 commits past `9726812`) on `merge/upstream-e2028e5`, keeping upstream ancestry. New upstream behavior: Kimi Code and Cursor providers (terminal and IDE worker), Hermes module-form detection, quiet-session grouping per provider with the card caret and inspector session-id matching, the GITEA heatmap, the pointer-reach gate (`card-stacking.test.ts`, `qmltestrunner`), the wallpaper aspect-ratio fix and headless render test, the 4.0.3-safe resolved-import gate, `INFOMARCHY_SKIP_REFINEMENT`, and the explicit shortcut setup section.
- Ten files conflicted (30 hunks). Fork choices: `InfoSettings.qml` routes `sessionGroups`, `sessionQuietMinutes` and `videoAudio` through `dashboard-state.ts` patches (validator extended), `InfoView.qml` keeps `displayPath` on the grouped card's cwd line, README keeps the fork document with three short notes, `collector.ts` runs Gitea, containers, Grok billing and Claude auth refresh in the same `Promise.all`, and the module list is `containers`, `media`, `gitea`.
- Rejected upstream `e11d25d`'s second and third commits: `FileView` watchers on `/home/j_kro/...` replacing the 4 s refresh timer, plus a duplicate `refresh()` the auto-merge produced. The fork keeps its Timer. Worth reporting upstream: on any other machine the desk refreshes once at start and never again.
- Upstream's Cursor snapshot test spawns `"bun"`, which here resolves to a mise shim that fails under the test's substituted `HOME`; changed to `process.execPath` like the fork's other spawns. Duplicated `INFOMARCHY_SKIP_GITHUB` env key removed.
