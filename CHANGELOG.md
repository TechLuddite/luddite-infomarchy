...
### Added
- **Quiet sessions group into one card, per provider and optionally.** Grok Bot runs its whole roster inside a single Electron process and the collector expands it into one card per bot, so a ten-bot roster costs ten cards — and on its own trips the dense-layout threshold (> 8), shrinking every Claude and Codex card on the desk to pay for bots nobody has touched in weeks. The desk now groups a provider's quiet sessions into one card that names a few of them with their idle times, still clickable, with right-click still opening that individual bot in the inspector; the chip expands the rest. On the desk this was written for it took SESSIONS from fifteen cards to six and turned dense mode off, so the surviving cards got their working directory, host, git and pid lines back. *Quiet* is decided per session, and the three attention states are deliberately not treated alike: **waiting** (an agent blocked on your answer) and **blocked** (a conflict, crash or failure) are requests, a request does not expire, and neither ever groups at any age. **done** — ready for review, or a bot holding replies you have not read — is a notification, and one nobody has looked at for a month has stopped being news, so it groups like any other idle card once it is past the window (an hour by default) and the card says `N to review` in the slot the group's uptime would have wasted. Nothing is hidden by this: `ai.attention` is built collector-side from the ungrouped list, so NEXT ACTIONS and the notifications still carry every signal. A group of one is refused, since it costs a click and saves no space. Groups are emitted at the end of the list rather than among the cards, because a group is taller than its neighbours and a Flow row is as tall as its tallest card — placed mid-grid it left a band of dead space under every short card sharing its row and pushed the rest into a row that read as leftovers. The rule is keyed by provider rather than hardcoded to Grok Bot — any app that fans one process out into a dozen sessions gets the same treatment — and Grok Bot is the only one that does that today, so it is the only one on by default. The toggle is on the cards, not in the module strip: that strip lists the panes below it, so a per-provider display rule was in the wrong place there. Grouping belongs to a provider, and a provider is named on every one of its own cards, so the name carries a disclosure caret and clicking it — on any card, including the group itself, so there is always a way back — turns grouping on and off. The caret is not a new icon language: WHAT CHANGED already spells "this expands and collapses" as a faint caption-sized `▴` / `▾`, and it means the same thing here — `▴` while a provider's cards are spread out, `▾` once they are grouped. It is `textFaint` until the pointer arrives, so a glance still reads as cards, and SESSIONS names the control and reports every running agent plus the quiet count in both states. The card reports the app's CPU, RAM and pid once, from the member that actually owns those counters, instead of the same process ten times over.
### Fixed
