...
### Added
- **Cursor sessions appear on the desk, from the IDE as well as the terminal.** `cursor-agent` run in a terminal is an ordinary session. The IDE is the case that matters in practice: when the conversation lives in Cursor rather than a terminal, the process that runs the agent is `cursor-agent worker` — one per open workspace, each bound to a single directory with its own pid, cwd and counters. It reads like a service and is not one, so it is deliberately **not** excluded the way Codex's `app-server` is; excluding it leaves a machine driving Cursor entirely from the IDE, which is most of them, with no Cursor session at all. Those cards are marked background and dimmed like Claude's `--bg` sessions, since they are driven from the window rather than a terminal, and clicking one focuses Cursor. The genuine one-shot management commands are excluded, and the subcommand is matched across the whole argument list rather than at `argv[1]`, because the IDE puts it after several flags (`--api-key`, `--endpoint`, …) and an `argv[1]` check would have missed the worker entirely. A worker lives as long as its Cursor window rather than as long as a conversation, so a Cursor card can sit on the desk for days — liveness is the process, decay is the last prompt, and the existing STALE machinery carries it. History is read from Cursor's own transcripts under `~/.cursor/projects/<dashed-cwd>/agent-transcripts/`, honouring `CURSOR_HOME`. Three things are resolved rather than guessed: the directory name replaces every `/` with `-`, which is ambiguous as soon as a segment contains a dash, so it is walked against the filesystem (`home-you-repos-four-monorepo` → `~/repos/four-monorepo`, never `~/repos/four/monorepo`) and an unresolvable name yields no project instead of a fabricated one; there is no timestamp field at all, so the `<timestamp>` tag the client injects into each user turn is parsed explicitly — `Date.parse` reads that string inconsistently and silently drops the offset on some builds — with the file mtime as the fallback so a chat still lands on the heatmap; and which chat a worker is on comes from the newest transcript under its own directory, because the generic inference only accepts a prompt within half an hour of launch, which fits a terminal agent and not a worker that outlives any one conversation (observed on a real desk: a worker launched at 16:02:50 whose chat opened at 16:34, one minute outside that window). Cursor also gives the desk its first real busy signal: a transcript ends with `{"type":"turn_ended"}`, so a trailing assistant turn without it means the agent is still working, rather than the terminal-title guessing every other provider needs — and for a worker the title could not help anyway, since it belongs to the IDE window that every workspace shares. That made the busy decision four-way with an order that matters, so it is now `sessionBusyState`: an agent that reports its own state wins, then a transcript the agent itself wrote, then the title, which for Grok is actively wrong. `crsr_` joins the redacted token prefixes. Known limitations, verified against real transcripts and pinned by tests: Cursor's hooks and rules inject their own turns as `role: "user"`, wrapped in `<user_query>`, timestamped and positioned exactly where a typed prompt goes, with no field to separate them — so they show up in RECENT TASKS alongside what a human asked, and filtering them would take a content heuristic that could quietly drop real prompts; every worker on a machine shares the one Cursor window, so clicking any of their cards focuses Cursor without switching it to that workspace; and Cursor publishes no local token or quota data, so it contributes no USAGE row.
