...
### Upstream sync — e2028e5 (2026-09-18)
- Integrated upstream `e2028e5`: Kimi Code and Cursor providers, Hermes module-form detection, quiet-session grouping, the Gitea heatmap, the pointer-reach gate, the aspect-ratio wallpaper fix, the 4.0.3 resolved-import gate, `INFOMARCHY_SKIP_REFINEMENT`, and the explicit shortcut setup section.
- Kept the fork's timed collector refresh. Upstream `e11d25d` replaced it with `FileView` watchers on one contributor's home path, which on any other machine never fire, so the desk would refresh once and then freeze.
- Preserved fork privacy, Web Mode, state locking, USAGE/billing, containers, media controls, layout and plugin identity.
...
- **Web Mode.** PHONE is now WEB. The LAN page matches the SUPER+D desk: two columns, theme colors, wallpaper center-cropped at 0.32 opacity. Narrow viewports stack with UP/DOWN reorder. Strip chips show and hide sections. Zoom controls scale the page in the tab. USAGE shows the TOKENS · 7 days graph only. PRIVACY masks recent-task prompts after four words, matching the desk. SETTINGS on the strip and an Infomarchy bar widget manage desk/web section visibility, tokens, a QR code, and extra CIDRs. Turning WEB off keeps tokens. Revoke a token to rotate it. MEDIA stays off the page.
- **MEDIA CONTROLS.** Last card in the right column. Live MPRIS title, artist, album, and player identity, with PREV / PLAY or PAUSE / NEXT. Prefers a playing player over `playerctld`. No album-art URL. Stream privacy leaves title, artist, album, and player identity in the clear. Not on the phone page.
...
- **Stream privacy (SUPER+SHIFT+I).** Toggles a persistent mask on the wallpaper and overlay: WAN, LAN, Wi-Fi SSID, `user@host`, GitHub `@login`, `/home/<user>` disk mounts, and hover window previews. Recent-task prompts keep the first four words and mask the rest. OSS project names, repos, and session topics stay. `omarchy-shell infomarchy togglePrivacy` (also `setPrivacy` / `getPrivacy`). One press enables it. Three presses within two seconds disable it. The module strip shows **PRIVACY ON** while it is active.
...
- **ABOUT.** The version sits at the quiet end of the legend line under the last card. Clicking it opens ABOUT: the version, a link to this fork, and a link to nixfred.com. The version is read from `manifest.json` and watched, so an in-place plugin update cannot leave a stale number on screen. `openUrl` refuses any address other than those two. Esc closes ABOUT before it closes the desk.
...
- **Herdr card click jumps to that agent's pane.** The `attached` gate is gone. `focusHerdrPane` still refuses ids it cannot validate.
- **A video wallpaper is whatever Omarchy says it is.** Both surfaces defer to `Util.isVideoPath` when the running Omarchy has it, and fall back to the extension list only where it does not.
...
- MEDIA CONTROLS keeps title, artist, album, and player identity while stream privacy is on. WAN, LAN, SSID, `user@host`, GitHub login, home mounts, window previews, and recent-task prompts still mask.
...
- Web USAGE shows the 7-day token chart and omits the $ chart. Privacy follows the desktop and is enforced before HTML/JSON leave the server; the former browser toggle is removed.
...
### Fixed
- **Failed HTTPS setup can be retried directly.** WEB FAILED and RETRY SETUP replace the misleading on-looking control. CHECK PREREQUISITES is inspection-only; active mappings no longer appear as preflight conflicts.
