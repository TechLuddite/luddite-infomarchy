...
### Added
- **Cursor sessions appear on the desk.** `cursor-agent` run in a terminal is now a card, with its project, topic, Recent Tasks rows, activity and a busy state. It is one binary with a dozen subcommands, so only the conversation counts: `agent` and `persist` are sessions, while `worker` — the self-hosted Cloud Agent worker the IDE launches to execute tool calls locally — and the one-shot management commands are excluded, the same shape as Codex's `app-server`. The subcommand is matched across the whole argument list rather than at `argv[1]`, because the IDE puts it after several flags (`--api-key`, `--endpoint`, …) and an `argv[1]` check would have missed the worker entirely. History is read from Cursor's own transcripts under `~/.cursor/projects/<dashed-cwd>/agent-transcripts/`, honouring `CURSOR_HOME`. Two quirks are resolved rather than guessed: the directory name replaces every `/` with `-`, which is ambiguous as soon as a segment contains a dash, so it is walked against the filesystem (`home-you-repos-four-monorepo` → `~/repos/four-monorepo`, never `~/repos/four/monorepo`) and an unresolvable name yields no project instead of a fabricated one; and there is no timestamp field at all, so the `<timestamp>` tag the client injects into each user turn is parsed explicitly — `Date.parse` reads that string inconsistently and silently drops the offset on some builds — with the file mtime as the fallback so a chat still lands on the heatmap. Cursor also gives the desk its first real busy signal: a transcript ends with `{"type":"turn_ended"}`, so a trailing assistant turn without it means the agent is still working, rather than the terminal-title guessing every other provider needs. `crsr_` joins the redacted token prefixes. Known limitation, verified against real transcripts and pinned by a test: Cursor's hooks and rules inject their own turns as `role: "user"`, wrapped in `<user_query>`, timestamped, and positioned exactly where a typed prompt goes, with no field to separate them — so they show up in RECENT TASKS alongside what a human asked. Filtering them would take a content heuristic that could drop real prompts, and observing a format beats guessing at one. Cursor publishes no local token or quota data, so it contributes no USAGE row.
