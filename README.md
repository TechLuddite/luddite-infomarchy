...
**Cursor.** `cursor-agent` is recognised when you run it in a terminal. It is one binary with a dozen subcommands, so only the conversation counts: `agent` and `persist` are sessions, while `worker` — the self-hosted Cloud Agent worker the IDE launches to run tool calls locally — and the one-shot management commands (`login`, `update`, `mcp`, `models`, …) are excluded, the same way Codex's `app-server` is. The subcommand is matched across the whole argument list rather than at `argv[1]`, because the IDE puts it after several flags.
...
Cursor is also the one provider with a real busy signal instead of a terminal-title guess: a transcript ends with `{"type":"turn_ended"}` once the agent has finished, so a trailing assistant turn without it means it is still working. **Known limitation:** Cursor's hooks and rules inject their own turns as `role: "user"`, wrapped in `<user_query>`, timestamped, and positioned exactly where a typed prompt goes — there is no field that separates them, so they appear in RECENT TASKS alongside what you actually asked. Filtering them would mean a content heuristic that could drop real prompts. Cursor publishes no local token or quota data either, so it contributes no USAGE row.
